...
Every program, when executed, it is handed an [[Arrays|array]] of [[String|strings]] from it's parent. the program may read the strings and change control flow based on them.

The strings are called "arguments", the purpose of which is to be a standard, convinent way to influence the behavior of the running program.

in other words, the program may behave differently based on a given list of strings.

Example: the program `seq` (available on every linux system) produces sequences of numbers. giving it the argument `10` like so: `seq 10`, causes it to print a sequence from 1 to 10. whereas `seq 5 3 20` prints every 3rd number from 5 to 20.
